# Methods: tumor_vs_normal

- command: `python3 project_b/scripts/run_pathway_enrichment.py --config project_b/brca_local_inputs.json --min-abs-delta-beta 0.2 --fdr-threshold 0.05 --comparisons tumor_vs_normal basal_vs_normal`
- CpG threshold: `fdr < 0.05` and `abs_delta_beta >= 0.2`
...
- gene-set source: Enrichr GMT endpoint cached to `pathway_enrichment/gene_sets/`
...
- gene-set library labels/version years are encoded in the Enrichr library names; cache download date is `2026-06-28` when first retrieved
