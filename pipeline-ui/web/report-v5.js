...
function queryUrl() {
  ...
  if (q.get('source') === 'run') {
    ...
    return `/api/v5/dashboard?source=run&id=${encodeURIComponent(id)}`;
  }
  return '/api/v5/dashboard?source=reference';
}
...
function renderHeader(d) {
  ...
  $('#report-kind').textContent = src.kind === 'reference' ? 'Breast published study · Version 5' : 'Sample / uploaded analysis · Version 5';
