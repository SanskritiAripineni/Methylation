...
function renderDashboard(d) {
  ...
  reportHost.querySelector('[data-body]').hidden = false;
  reportHost.querySelector('[data-state]').hidden = true;
  $('#report-frame').src = reportUrl(src);
  $('#report-label').innerHTML = `<b>${esc(src.title)}</b> — ${
    src.kind === 'reference' ? 'breast published study' : 'report generated from this selected run'}.`;
  $('#report-variant').hidden = true;
...
function reportUrl(src) {
  return src.kind === 'reference'
    ? '/report-v5.html?source=reference'
    : `/report-v5.html?source=run&id=${encodeURIComponent(src.id)}`;
}
