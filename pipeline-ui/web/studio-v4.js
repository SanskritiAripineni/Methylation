 ...
 * after this one. They are not copied per version - that is how the published
...
function renderDashboard(d) {
  ...
  /* ---- report ---- */
  const rep = d.report;
  viz.paintState(panel('report'), rep);
  if (rep.state === 'ok') {
    $('#report-frame').src = rep.url;
    $('#report-label').innerHTML = `<b>${esc(src.title)}</b>${
      src.kind === 'run' ? ` — ${esc(src.tier_label)}, generated by this run.` : ''}`;
    const variants = rep.variants || [];
    $('#report-variant').hidden = variants.length < 2;
    $('#report-variant').innerHTML = variants
      .map(o => `<option value="${esc(o.url)}">${esc(o.label)}</option>`).join('');
  } else {
    $('#report-frame').removeAttribute('src');
    $('#report-variant').hidden = true;
    $('#report-label').textContent = 'No report for this selection.';
  }
...
function renderSourcePick() {
  ...
  const watched = state.runId && state.snap
    && !['running', 'queued'].includes(state.snap.status);
  $('#sourcepick').innerHTML = `
    <button class="src ${sel.kind === 'run' ? 'on' : ''}" data-src="run"
      ${watched ? '' : 'disabled'}>${
  watched ? esc(state.snap.label || 'Your run') : 'Your run'}</button>
    <button class="src ${sel.kind === 'reference' ? 'on' : ''}" data-src="reference">
      Published study</button>`;
  $$('#sourcepick [data-src]').forEach(b => b.addEventListener('click', () => {
    if (b.disabled) return;
    if (b.dataset.src === 'reference') select({ kind: 'reference', id: 'published-study' });
    else if (state.runId) select({ kind: 'run', id: state.runId });
  }));
}
